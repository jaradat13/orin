...
def calculate_snapshot_delta(db_path: Path, base_id: int, target_id: int) -> dict:
    ...
    with storage.get_connection() as conn:
        ...
        target_ports = {r["port"]: r for r in cursor.fetchall()}
        
        for port in (target_ports.keys() - {bp[0] for bp in base_ports}):
            row = target_ports[port]
            delta_report["new_ports"].append({"port": port, "protocol": row["protocol"], "process": row["process_name"]})
